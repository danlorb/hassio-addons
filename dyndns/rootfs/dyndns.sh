...
if [[ "${record_id}" = "" ]]; then
  ...
  curl -s -k -X "POST" "https://dns.hetzner.com/api/v1/records" \
...
# check if update is needed
  cur_dyn_addr=`curl -k -s "https://dns.hetzner.com/api/v1/records/${record_id}" -H 'Auth-API-Token: '${auth_api_token} | jq --raw-output '.record.value'`
...
# update existing record
  if [[ $cur_pub_addr == $cur_dyn_addr ]]; then
    ...
    curl -k -s -X "PUT" "https://dns.hetzner.com/api/v1/records/${record_id}" \
         -H 'Content-Type: application/json' \
         -H 'Auth-API-Token: '${auth_api_token} \
         -d $'{
           "value": "'${cur_pub_addr}'",
